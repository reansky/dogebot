(() => {
  ...
  bar.innerHTML = '<strong style="color:#c2ff00">ChatGPT Terminal</strong><span data-chatgpt-status style="color:#8e9991">CHECKING SERVER</span>';
  pane.prepend(bar);
  pane.querySelectorAll('[data-hub-tab="chat"]').forEach((tab) => { tab.textContent = 'ChatGPT Terminal'; });
  input.placeholder = 'Ask ChatGPT about DOGEBOT PACK';

  const status = bar.querySelector('[data-chatgpt-status]');
  ...
  fetch('/api/chat').then((response) => response.json()).then((data) => {
    if (data.configured) {
      status.textContent = 'ONLINE / READ-ONLY';
      status.style.color = '#68f29a';
      append('SYSTEM', 'ChatGPT is connected. This terminal is read-only and cannot execute transactions.', '#68f29a');
    } else {
      status.textContent = 'SERVER KEY REQUIRED';
      status.style.color = '#f5a623';
      append('SYSTEM', 'ChatGPT Terminal is installed but the server API key has not been configured yet.', '#f5a623');
    }
  }).catch(() => {
  ...
  async function ask(text) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.error || 'ChatGPT is unavailable.');
      history.push({ role: 'assistant', content: data.reply });
      append('CHATGPT', data.reply, '#c2ff00');
      status.textContent = 'ONLINE / READ-ONLY';
    } catch (error) {
