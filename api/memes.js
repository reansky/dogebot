...
module.exports = async function handler(req, res) {
  try {
    ...
    if (!(await rateLimit(ipKey(req, 'meme-create', viewer), 3, 3600))) {
      return json(res, 429, { error: 'Meme submissions are limited to three per hour.' });
    }
    ...
    if (!buffer.length || buffer.length > MAX_BYTES) return json(res, 413, { error: 'Meme images must be 1.5 MB or smaller.' });
