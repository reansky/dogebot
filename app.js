...
aiForm.onsubmit=event=>{event.preventDefault();const text=aiInput.value;aiInput.value='';askAi(text)};aiInput.onkeydown=event=>{if(event.key==='Enter'&&!event.shiftKey){event.preventDefault();aiForm.requestSubmit()}};document.querySelectorAll('[data-ai-prompt]').forEach(button=>button.onclick=()=>{aiInput.value=button.dataset.aiPrompt;aiForm.requestSubmit()});function setAiOpen(open){aiOverlay.classList.toggle('open',open);aiOverlay.setAttribute('aria-hidden',String(!open));aiLaunch.setAttribute('aria-expanded',String(open))}aiLaunch.onclick=()=>setAiOpen(true);aiClose.onclick=()=>setAiOpen(false);aiOverlay.onclick=event=>{if(event.target===aiOverlay)setAiOpen(false)};document.addEventListener('keydown',event=>{if(event.key==='Escape')setAiOpen(false)});document.getElementById('aiClear').onclick=()=>{aiHistory=[{role:'assistant',content:'Chat cleared. Ask me about the DOGEBOT PACK project.'}];saveAi();renderAi()};
 ...
 const marquee=document.createElement('div');marquee.className='pack-marquee';marquee.setAttribute('aria-label','DOGEBOT Pack community message');marquee.innerHTML='<div class="pack-marquee-track"><span>BUY $NVDA</span><span>HOLD $NVDA</span><span>JOIN THE PACK</span><span>BUY $NVDA</span><span>HOLD $NVDA</span><span>JOIN THE PACK</span></div>';document.querySelector('.workflow-strip')?.after(marquee);
...
const explainNVDAMarket = localAi;
localAi = (question) => /nvda|nvda|pair|market/i.test(String(question || ''))
  ? 'This is the NVDA MARKET panel for NVDA (NVDA). It is read-only: no live DOGEBOT/NVDA trading pair, trades, swaps, or buy recommendation is active.'
  : explainNVDAMarket(question);
...
(() => {
  ...
  if (marketCopy) marketCopy.textContent = '$NVDA includes a clearly labeled NVDA market panel ($NVDA) on Robinhood Chain. Live price, market cap, liquidity, volume, and holder data are read-only public signals; no trades or buy recommendation are active.';
...
(() => {
  ...
  const quoteUpdates = {
    ...
    ROUNDS: ['0', 'no rounds enabled'],
    FEES: ['0%', 'no active transfer tax'],
  };
...
(() => {
  ...
  localAi = (question) => /price|market|token|contract|nvda|nvda/i.test(String(question || ''))
...
// Keep section numbers and navigation in the same order as the page.
(() => {
  ...
  const labels = {
    rewards: '01 / THE DOGEBOT PACK SYSTEM',
    dogebot: '02 / NVDA MARKET',
    pack: '03 / WHAT THE PACK IS BUILDING',
  ...
  const reorderLinks = (selector) => {
    ...
    [...parent.querySelectorAll('a')]
      .sort((a, b) => {
        const ai = linkOrder.indexOf(a.getAttribute('href'));
        const bi = linkOrder.indexOf(b.getAttribute('href'));
        return (ai < 0 ? 99 : ai) - (bi < 0 ? 99 : bi);
      })
      .forEach((link) => parent.append(link));
  };
  reorderLinks('.nav-links');
